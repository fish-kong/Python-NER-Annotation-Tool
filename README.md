...
本工具专为文本标注任务设计，提供了直观的图形界面，支持多种实体类型的标注。标注结果以BIO格式保存，适用于自然语言处理和机器学习模型的训练数据准备。
...
3. **BIO格式导出**
   - 自动将标注结果转换为BIO格式（B-Begin, I-Inside, E-End, S-Single）
...
2. 选中"220kV"，点击"电压等级"按钮（红色）
3. 选中"主变压器"，点击"装置类型"按钮（粉色）
4. 点击"保存文件为CSV"
...
2. **文件编码**：所有CSV文件使用UTF-8编码
3. **标签覆盖**：对同一文本区域应用新标签时，会自动移除旧标签
4. **文件保存**：保存的文件会自动创建`result/`目录（如果不存在）
